...
import { GraduationCap, Hand, LayoutIcon, Settings as SettingsIcon } from "lucide-react";
...
function SideNav() {
  ...
  const menuList = [
    ...
    { id: 4, name: "Settings", icon: SettingsIcon, path: "/dashboard/settings" }, // ✅ Fixed path
  ];
    ...
    <div className="border shadow-md h-screen p-5 border-y-red-900">
      ...
      {menuList.map((menu) => (
        <Link key={menu.id} href={menu.path} className="block">
          <h2
            className={`flex items-center gap-3 text-md p-4 text-slate-500 hover:bg-primary hover:text-white cursor-pointer rounded-lg my-3 ${
